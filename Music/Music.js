...
import { Text, View, Animated, Image, TextInput, TouchableHighlight ,FlatList,Alert, ImageBackground,DeviceEventEmitter } from 'react-native'
...
const musicUrl="http://82.156.12.58:808/music/"
...
export default class App12 extends Component {
    ...
    _playOrPause=()=>{
        //涉及动画对象
        let an=Animated.loop(Animated.timing(this.state.rotate,{useNativeDriver:true,duration:6000,toValue:this.rv+360,easing:linear}))
    ...
    // 
    _likeMusic=(id)=>{
        console.log("linkemusic_function")
        fetch(`${dataUrl}/${id}`,{
            method:'PATCH',
        ...
        .then(()=>{
            DeviceEventEmitter.emit("addlike")
        }
        )
    ...
    render() {
        ...
        return (
            <Provider>
                <ImageBackground source={{uri:mus[mIndex]?.Bg}} style={{flex:1}}>
                <View style={{flex:1}}>
                    <TopView/>
                    {/* 歌曲及歌手 */}
                    <View style={{backgroundColor:"rgba(51,51,51,0.7)"}}>
                        ...
                        <Text style={{textAlign:'center',color:'white',fontSize:15,marginTop:3,marginBottom:10,paddingBottom:5}}>{mus[mIndex]?.Singer}</Text>
                    </View>
                    {/* 分享按钮 */}
                    ...
                    <View style={{alignItems:'center'}}>
                        ...
                        <Animated.Image style={[style,{width:260,height:260,borderRadius:130,alignSelf:'center'}]} source={{uri:mus[mIndex]?.Post}}/>
                    ...
                    <View style={{height:'48%',justifyContent:'space-evenly',flexDirection:'row',alignItems:'center'}}>
                        ...
                        {/* 添加喜爱 */}
                        <View>
                            <TouchableHighlight activeOpacity={0.1} underlayColor={'rgba(0,0,0,0)'} onPress={mus[mIndex]?.Like?()=>this._unlikeMusic(mus[mIndex]?.id):()=>this._likeMusic(mus[mIndex]?.id)} style={{opacity:0.85}}>
                                <AntDesign name={'heart'} size={32} color={mus[mIndex]?.Like?'red':'aliceblue'}/>
                            </TouchableHighlight>
                        </View>
                        {/* 上一首 */}
                        <View>
                    ...
                    <Modal visible={this.state.visible2} popup animationType='slide-down' maskClosable onClose={()=>this.setState({visible2:false})} transparent={true} style={{width:300,height:460,alignItems:'center'}}>
                        ...
                        <TouchableHighlight activeOpacity={0.1} underlayColor={'white'} style={{justifyContent:'flex-end',alignItems:'flex-start'}} onPress={()=>this.setState({visible:false})}>
                            <View>
                                <Image source={{uri:mus[mIndex]?.Post}} style={{height:250,width:250,borderRadius:10}}/>
